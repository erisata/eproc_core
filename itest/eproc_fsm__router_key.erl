...
-export([new/0, add_key/4, next_state/1, finalise/1]).
...
add_key(FsmRef, Key, Scope, Opts) ->
    eproc_fsm:sync_send_event(FsmRef, {add_key, Key, Scope, Opts}).
...
handle_state({active, _}, {sync, _From, {add_key, Key, Scope, Opts}}, StateData) ->
    Reply = case eproc_router:add_key(Key, Scope, Opts) of
