...
%     end,
    ...
    From = 1,
    Count = 99,
    {ok, Res00} = eproc_store:get_instance(Store, {filter, From+2, 3, []}, current),
    {ok, Res01} = eproc_store:get_instance(Store, {filter, From, Count, []}, current),
    {ok, Res02} = eproc_store:get_instance(Store, {filter, From, Count, [{id, IID1}]}, current),
    {ok, Res03} = eproc_store:get_instance(Store, {filter, From, Count, [{name, testing_name}]}, current),
    {ok, Res04} = eproc_store:get_instance(Store, {filter, From, Count, [{last_trn, undefined, OldDate2}]}, current),
    {ok, Res05} = eproc_store:get_instance(Store, {filter, From, Count, [{last_trn, OldDate2, undefined}]}, current),
    {ok, Res06} = eproc_store:get_instance(Store, {filter, From, Count, [{last_trn, OldDate2, OldDate4}]}, current),
    {ok, Res07} = eproc_store:get_instance(Store, {filter, From, Count, [{created, undefined, OldDate2}]}, current),
    {ok, Res08} = eproc_store:get_instance(Store, {filter, From, Count, [{created, OldDate2, undefined}]}, current),
    {ok, Res09} = eproc_store:get_instance(Store, {filter, From, Count, [{created, OldDate2, OldDate4}]}, current),
    {ok, Res10} = eproc_store:get_instance(Store, {filter, From, Count, [{tags, [{<<"123">>, <<"cust_nr">>}]}]}, current),
    {ok, Res11} = eproc_store:get_instance(Store, {filter, From, Count, [{tags, [{<<"123">>, <<"cust_nr">>},{<<"rejected">>, <<"resolution">>}]}]}, current),
    {ok, Res12} = eproc_store:get_instance(Store, {filter, From, Count, [{tags, [{<<"123">>, <<"cust_nr">>}]}, {tags,[{<<"rejected">>, <<"resolution">>}]}]}, current),
    {ok, Res13} = eproc_store:get_instance(Store, {filter, From, Count, [{module, some_other_fsm}]}, current),
    {ok, Res14} = eproc_store:get_instance(Store, {filter, From, Count, [{status, resuming}]}, current),
    {ok, Res15} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}}]}, current),
    {ok, Res16} = eproc_store:get_instance(Store, {filter, From, Count, [{age, [{2,day},{1,hour}]}]}, current),
    {ok, Res16} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}}, {age, [{2,day},{1,hour}]}]}, current),
    % Evaluating results
    Req = fun(List) ->
    ...
    {ok, Res50} = eproc_store:get_instance(Store, {filter, From, Count, [{id, IID1},{name, testing_name}]}, current),
    {ok, Res51} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}},{id, IID1}]}, current),
    {ok, Res52} = eproc_store:get_instance(Store, {filter, From, Count, [{created, OldDate2, undefined},{tags, [{<<"123">>, <<"cust_nr">>}]}]}, current),
    {ok, Res53} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}},{created, OldDate2, undefined}]}, current),
    % Evaluating results
