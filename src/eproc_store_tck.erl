...
%     end,
    ...
    {ok, {_, _, []}} = eproc_store:get_instance(Store, {filter, {From, Count}, [{id, non_existent_id}]}, header),
    {ok, {_, _, []}} = eproc_store:get_instance(Store, {filter, {From, Count}, [{name, non_existent_name}]}, header),
    {ok, {_, _, []}} = eproc_store:get_instance(Store, {filter, {From, Count}, [{id, IID3},{name, testing_name}]}, header),
    {ok, {_, _, []}} = eproc_store:get_instance(Store, {filter, {From, Count}, [{module, some_other_fsm}, {module, [another_fsm]}]}, header),
    {ok, {_, _, []}} = eproc_store:get_instance(Store, {filter, {From, Count}, [{status, [resuming]}, {status, killed}]}, header),
