...
%     end,
    ...
    From = 1,
    Count = 99,
    {ok, Res00} = eproc_store:get_instance(Store, {filter, From+2, 3, []}, header),
    {ok, Res01} = eproc_store:get_instance(Store, {filter, From, Count, []}, header),
    {ok, Res02} = eproc_store:get_instance(Store, {filter, From, Count, [{id, IID1}]}, header),
    {ok, Res03} = eproc_store:get_instance(Store, {filter, From, Count, [{name, testing_name}]}, header),
    {ok, Res04} = eproc_store:get_instance(Store, {filter, From, Count, [{last_trn, undefined, OldDate2}]}, header),
    {ok, Res05} = eproc_store:get_instance(Store, {filter, From, Count, [{last_trn, OldDate2, undefined}]}, header),
    {ok, Res06} = eproc_store:get_instance(Store, {filter, From, Count, [{last_trn, OldDate2, OldDate4}]}, header),
    {ok, Res07} = eproc_store:get_instance(Store, {filter, From, Count, [{created, undefined, OldDate2}]}, header),
    {ok, Res08} = eproc_store:get_instance(Store, {filter, From, Count, [{created, OldDate2, undefined}]}, header),
    {ok, Res09} = eproc_store:get_instance(Store, {filter, From, Count, [{created, OldDate2, OldDate4}]}, header),
    {ok, Res10} = eproc_store:get_instance(Store, {filter, From, Count, [{tags, [{<<"123">>, <<"cust_nr">>}]}]}, header),
    {ok, Res11} = eproc_store:get_instance(Store, {filter, From, Count, [{tags, [{<<"123">>, <<"cust_nr">>},{<<"rejected">>, <<"resolution">>}]}]}, header),
    {ok, Res12} = eproc_store:get_instance(Store, {filter, From, Count, [{tags, [{<<"123">>, <<"cust_nr">>}]}, {tags,[{<<"rejected">>, <<"resolution">>}]}]}, header),
    {ok, Res13} = eproc_store:get_instance(Store, {filter, From, Count, [{module, some_other_fsm}]}, header),
    {ok, Res14} = eproc_store:get_instance(Store, {filter, From, Count, [{status, resuming}]}, header),
    {ok, Res15} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}}]}, header),
    {ok, Res16} = eproc_store:get_instance(Store, {filter, From, Count, [{age, [{2,day},{1,hour}]}]}, header),
    {ok, Res16} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}}, {age, [{2,day},{1,hour}]}]}, header),
    % Evaluating results
    Req = fun(List) ->
    ...
    {ok, Res50} = eproc_store:get_instance(Store, {filter, From, Count, [{id, IID1},{name, testing_name}]}, header),
    {ok, Res51} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}},{id, IID1}]}, header),
    {ok, Res52} = eproc_store:get_instance(Store, {filter, From, Count, [{created, OldDate2, undefined},{tags, [{<<"123">>, <<"cust_nr">>}]}]}, header),
    {ok, Res53} = eproc_store:get_instance(Store, {filter, From, Count, [{age, {12,hour}},{created, OldDate2, undefined}]}, header),
    % Evaluating results
    ...
    {ok, []} = eproc_store:get_instance(Store, {filter, From, Count, [{id, non_existent_id}]}, header),
    {ok, []} = eproc_store:get_instance(Store, {filter, From, Count, [{name, non_existent_name}]}, header),
    {ok, []} = eproc_store:get_instance(Store, {filter, From, Count, [{id, IID3},{name, testing_name}]}, header),
    ok.
